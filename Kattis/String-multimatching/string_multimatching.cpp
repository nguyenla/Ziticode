...
struct SuffixTreeNode {
    ...
    vector<int> findAllOccurrences(const string& input, const string& pattern) {
        ...
        while (pat_pos < pattern.length()) {
            int i = cur->start_index;
            while (i <= cur->end_index && pat_pos < pattern.length() && i >= 0) {
                if (pat_pos < pattern.length() && pattern[pat_pos] != input[i]) {
                    return results;
                }
                ++pat_pos;
                ++i;
            ...
            if (pat_pos < pattern.length()) {
                auto it = cur->children.find(pattern[pat_pos]);
        ...
        // Traverse cur to find all leaves
        queue<SFTN*> node_queue;
        node_queue.push(cur);
        while (!node_queue.empty()) {
            ...
            if (node->children.size() == 0) {
                results.push_back(node->end_index + 1 - node->depth);
            }
        }
...
int main() {
    ...
    while (cin >> num_patterns) {
        ...
        string input, text;
        getline(cin, input); // Throw away end of line
        for (int i = 0; i < num_patterns; ++i) {
            getline(cin, input);
            patterns.push_back(input);
        }
        getline(cin, text);
        auto suffix_tree = constructSuffixTree(text + "$");
        // suffix_tree->print(text);
        
        for (int i = 0; i < num_patterns; ++i) {
            auto occurrences = suffix_tree->findAllOccurrences(text, patterns[i]);
            print(occurrences);
        }
    }
