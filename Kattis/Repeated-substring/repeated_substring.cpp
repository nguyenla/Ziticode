...
struct SuffixTreeNode {
    ...
    int findNumRepeatedSubstrings() {
        int sum = 0;
        for (const auto& child : this->children) {
            if (child.second->children.size() > 0) {
                sum += child.second->findNumRepeatedSubstrings();
            }
        }
        // If this node is not the root and has children
        if (this->children.size() > 0 && this->start_index != -1) {
            sum += this->depth - this->parent->depth;
        }
        return sum;
    }
...
int main() {
    ...
    for (int i = 0; i < num_input; ++i) {
        ...
        cout << suffix_tree->findNumRepeatedSubstrings() << "\n";
    }
